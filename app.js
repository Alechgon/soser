...
function renderHome() {
  ...
  content.innerHTML = `<div class="screen"><div style="flex:0 0 auto">
    ...
    <div class="home-actions">
      <button class="emerg-action" id="aEmerg"><div class="pulse"></div><div class="pa-ic">🚨</div><div><h3>Emergencia</h3></div><div class="pa-arrow">›</div></button>
      <button class="primary-action" id="aAdd"><div class="pa-ic">➕</div><div><h3>Agregar caso</h3></div><div class="pa-arrow">›</div></button>
      <button class="primary-action rep" id="aReports"><div class="pa-ic">📋</div><div><h3>Reportes generados</h3></div><div class="pa-arrow">›</div></button>
    </div>
...
let repKpiFilter = null;      // null | 'total' | 'der' | 'fin' — filtro por KPI presionado
...
async function renderReports() {
  ...
  repTab = 'mios'; repFilterEst = null; repKpiFilter = null;
...
function paintReports() {
  ...
  content.innerHTML = `<div class="screen">
    <div class="rep-head">
      <div class="rep-toolbar">
        <div class="tabs">
          <button class="tab ${repTab === 'mios' ? 'sel' : ''}" data-tab="mios">Mis casos</button>
      ...
      <div class="kpis">
        <button class="kpi ${repKpiFilter === 'total' ? 'kpisel' : ''}" data-kpi="total"><div class="bar"></div><div class="n">${total}</div><div class="l">${(repTab === 'general' && repFilterEst) ? 'Casos' : 'Total'}</div></button>
        <button class="kpi der ${repKpiFilter === 'der' ? 'kpisel' : ''}" data-kpi="der"><div class="bar"></div><div class="n">${derivados}</div><div class="l">Derivados</div></button>
        <button class="kpi sol ${repKpiFilter === 'fin' ? 'kpisel' : ''}" data-kpi="fin"><div class="bar"></div><div class="n">${solucion}</div><div class="l">Solucionados</div></button>
      </div>
      ${repKpiFilter ? `<div class="kpi-filter-note">Mostrando solo: <b>${repKpiFilter === 'total' ? 'todos' : (repKpiFilter === 'der' ? 'derivados' : 'solucionados')}</b> · <span id="clrKpi" style="text-decoration:underline;cursor:pointer">quitar filtro</span></div>` : ''}
      ${kpiSub}
      ${searchBlock}
    </div>
  ...
  // KPIs presionables: filtran la lista; volver a tocar el mismo lo quita
  $$('.kpi[data-kpi]').forEach(k => k.onclick = () => {
    const f = k.dataset.kpi;
    repKpiFilter = (repKpiFilter === f || f === 'total') ? (f === 'total' ? null : null) : f;
    if (f === 'total') repKpiFilter = null;
    paintReports();
  });
  const clrK = $('#clrKpi'); if (clrK) clrK.onclick = () => { repKpiFilter = null; paintReports(); };
  bindRepList();
  if (repTab === 'general') {
...
function pickGeneralEst(e) { repFilterEst = e; repKpiFilter = null; paintReports(); }

async function switchTab(tab) {
  if (repTab === tab) return;
  repTab = tab; repFilterEst = null; repKpiFilter = null;
  if (tab === 'general') {
    paintReports();
...
function renderRepList(scope) {
  let list = sortedScope(scope);
  // filtro por KPI presionado
  if (repKpiFilter === 'der') list = list.filter(r => !isBorrado(r) && estadoDe(r).k === 'der');
  else if (repKpiFilter === 'fin') list = list.filter(r => !isBorrado(r) && estadoDe(r).k === 'fin');
  lastRenderedList = list;
  if (!list.length) {
    if (repKpiFilter) return `<div class="empty"><div class="ic">🔎</div><p>Sin casos ${repKpiFilter === 'der' ? 'derivados' : 'solucionados'} en esta vista.</p></div>`;
  ...
  return list.map((r, i) => {
    ...
    return `<div class="rep ${borrado ? 'deleting' : ''}" data-open="${i}" style="cursor:pointer">
...
function bindRepList() {
  $$('#repList .rdel').forEach(btn => btn.onclick = e => { e.stopPropagation(); openDelete(btn.dataset.del); });
  $$('#repList .rverif').forEach(btn => btn.onclick = e => { e.stopPropagation(); openViewer(verifList(lastRenderedList[+btn.dataset.verif])); });
  $$('#repList .rsend').forEach(btn => btn.onclick = e => { e.stopPropagation(); resendPending(lastRenderedList[+btn.dataset.send], btn); });
  $$('#repList .rep[data-open]').forEach(card => card.onclick = () => openCaseDetail(lastRenderedList[+card.dataset.open]));
}

/* Detalle del caso: cabecera + verificadores del encargado y del técnico + comentario de reparación.
   El visor de verificadores usa openViewer (robusto: Drive thumb/iframe/fallback, nunca se pega). */
let detailReturn = null;
function openCaseDetail(r) {
  if (!r) return;
  detailReturn = paintReports;
  showNav(false); $('#btnHome').classList.remove('hidden'); $('#btnHome').onclick = renderHome;
  const st = estadoDe(r);
  const em = /emergencia/i.test(r.cat || r.categoria || '');
  const vEnc = verifList(r);                          // verificadores del encargado (y media local)
  const vTec = verifListTecnico(r);                   // verificadores del técnico (si solucionado)
  const coment = r.comentarioTecnico || r.comentario || '';
  const solPor = r.solucionadoPor || '';
  content.innerHTML = `<div class="screen"><div style="flex:1;overflow-y:auto;display:flex;flex-direction:column">
    <div class="bubble-head ${em ? 'em' : ''}">
      <div class="bh-top"><div class="bh-id">${esc(r.id)}</div><div class="bh-state">${st.t}</div></div>
      <h3>${em ? '🚨 ' : ''}${esc(r.nom || r.establecimiento)} · RBD ${esc(r.rbd)}</h3>
      <div class="bh-meta"><span>${esc(r.cat || r.categoria || '')}</span><span>${esc(r.fecha || '')}</span>${r.encargado ? `<span>👷 ${esc(r.encargado)}</span>` : ''}${r.derivadoA ? `<span>↗ ${esc(r.derivadoA)}</span>` : ''}</div>
      <div class="bh-desc">${esc(r.desc || r.descripcion || 'Sin descripción')}</div>
    </div>

    ${vEnc.length ? `<div class="d-lbl">Verificadores del encargado</div>
      <div class="d-strip" id="stripEnc">${vEnc.map((m, i) => dThumb(m, i)).join('')}</div>` : ''}

    ${st.k === 'fin' ? `<div class="d-solbox">
        <div class="d-lbl" style="margin-top:0">Reparación${solPor ? ' · ' + esc(solPor) : ''}</div>
        ${coment ? `<div class="d-coment">📝 ${esc(coment)}</div>` : '<div class="d-coment muted">Sin comentario de reparación.</div>'}
        ${r.fechaSolucionado ? `<div class="d-fecha">✓ Solucionado el ${esc(r.fechaSolucionado)}${r.tiempoResolucion ? ' · ' + esc(r.tiempoResolucion) : ''}</div>` : ''}
        ${vTec.length ? `<div class="d-lbl">Verificadores del técnico</div><div class="d-strip" id="stripTec">${vTec.map((m, i) => dThumb(m, i)).join('')}</div>` : ''}
      </div>` : ''}
  </div></div>`;

  const se = $('#stripEnc'); if (se) $$('.d-vs', se).forEach((el, i) => el.onclick = () => openViewer(vEnc, i));
  const stc = $('#stripTec'); if (stc) $$('.d-vs', stc).forEach((el, i) => el.onclick = () => openViewer(vTec, i));

  showNav(true);
  navwrap.querySelector('.inner').innerHTML = `<button class="btn ghost" id="dBack">‹ Volver</button>`;
  $('#dBack').onclick = () => { navwrap.querySelector('.inner').innerHTML = ''; paintReports(); };
}
function dThumb(m, i) {
  const thumb = m.driveId ? driveThumb(m.driveId) : m.url;
  return `<div class="d-vs" data-i="${i}">${m.type === 'video' ? `<video src="${esc(m.url)}" muted></video><div class="d-vt">▶</div>` : `<img src="${esc(thumb)}" alt="" loading="lazy" onerror="this.style.opacity=.25">`}</div>`;
}
/* Verificadores del técnico: vienen en r.verificadoresTecnico (mismo formato "tipo: nombre -> url") */
function verifListTecnico(r) {
  const out = [];
  const raw = r.verificadoresTecnico || '';
  if (!raw) return out;
