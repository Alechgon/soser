...
function renderHome(){
  ...
  content.innerHTML=`<div class="screen"><div class="fill" style="justify-content:flex-start">
    <div class="hero" style="padding:24px 12px 6px"><div class="mark">${LOGO_SVG}</div><h1>Gestión de Casos</h1>
    ...
    <div class="home-actions">
      ...
      <button class="primary-action" id="aReports" style="background:linear-gradient(120deg,#3a3a3a,#2a2a2a)"><div class="pa-ic" style="background:var(--grad-soft)">📋</div>
        <div><h3>Reportes generados</h3><p>${nrep} registrado(s) · visados y derivaciones</p></div><div class="pa-arrow">›</div></button>
    </div>
    ${cfgOk?'':'<div class="cfg-warn">Configura primero (botón inferior izquierdo) el <b>Sheet</b> y el <b>nombre de encargado</b>.</div>'}
  </div></div>
  <div class="cfg-corner" id="aCfg"><span class="cg-ic">⚙️</span> Configuración</div>`;
...
function askPin(onOk){
  ...
  navbar.classList.add('hidden');$('#btnHome').classList.remove('hidden');$('#btnHome').onclick=renderHome;
  content.innerHTML=`<div class="screen"><div class="fill" style="justify-content:center">
    <div class="card" style="max-width:360px;margin:0 auto;width:100%">
      <div class="eyebrow"><b>Configuración</b></div>
      <h2 class="q" style="text-align:center">Ingresa la clave</h2>
      ...
      <div class="pin-grid" id="pinGrid">
        ...
        <button class="pin-key" data-k="ok" style="background:var(--grad);color:#fff">✓</button>
      </div>
    </div>
  </div></div>`;
  ...
  $$('#pinGrid .pin-key').forEach(b=>b.onclick=()=>{
    const k=b.dataset.k;
    if(k==='del')entered=entered.slice(0,-1);
    else if(k==='ok')return check();
    else if(entered.length<9)entered+=k;
    dots();
    if(entered.length===9)check();
  });
}
...
async function renderReports(){
  navbar.classList.remove('hidden');$('#btnHome').classList.remove('hidden');$('#btnHome').onclick=renderHome;
  $('#btnNext').classList.add('hidden');$('#btnBack').onclick=renderHome;
  // pinta primero lo local, luego intenta refrescar desde el Sheet
  const local=loadReports().filter(r=>r.encargado===CFG.encargado).sort((a,b)=>b.ts-a.ts);
  const paint=(list,fromSheet)=>{
    content.innerHTML=`<div class="screen"><div class="fill" style="overflow-y:auto">
      <div class="eyebrow" style="margin:0 4px 8px"><b>Reportes generados</b> <span class="grp">· ${CFG.encargado||'—'}</span></div>
      <h2 class="q" style="margin:0 4px 14px">${list.length} reporte(s) ${fromSheet?'<span style="font-size:12px;color:var(--green-d)">· sincronizado</span>':'<span style="font-size:12px;color:var(--muted)">· local</span>'}</h2>
      ${list.length?list.map(r=>{
        const visado=(r.visado||'').toString().trim();
        const deriv=(r.derivadoA||'').toString().trim();
        const st = r.enviado===false ? '<span class="rstatus pend">⚠ Pendiente de envío</span>'
                 : visado ? `<span class="rstatus ok">✓ Visado: ${visado}</span>`
                 : '<span class="rstatus pend">⏳ Sin visar</span>';
        return `<div class="rep"><div class="rh"><span class="rid">${r.id}</span><span class="rcat">${r.cat||r.categoria||''}</span></div>
          <div class="rtitle">${(r.nom||r.establecimiento||'')} · RBD ${r.rbd}</div>
          <div class="rmeta">${r.fecha} · ${(r.desc||r.descripcion||'').slice(0,60)}</div>
          ${st}${deriv?`<span class="rderiv">↗ Derivado a: ${deriv}</span>`:''}
          ${r.enviado===false?`<div style="margin-top:8px"><button class="btn accent" style="padding:9px 14px;font-size:13px" data-resend="${r.id}">Reintentar envío</button></div>`:''}
        </div>`;}).join(''):'<p class="note">Aún no hay reportes.</p>'}
    </div></div>
    <div class="cfg-corner" id="aCfg"><span class="cg-ic">⚙️</span> Configuración</div>`;
  ...
  paint(local,false);
  // refrescar desde Sheet (sobrevive a borrado de caché)
  const remote=await fetchReportsFromSheet();
  if(remote){
    // fusiona: remoto manda; conserva pendientes locales no enviados
    const pend=local.filter(r=>r.enviado===false);
...
async function fetchReportsFromSheet(){
  ...
  try{
    const url=CFG.sheetUrl+'?encargado='+encodeURIComponent(CFG.encargado);
    const res=await fetch(url,{method:'GET'});
    const data=await res.json();
    if(data&&data.ok&&Array.isArray(data.reportes)) return data.reportes.reverse();
    return null;
  }catch(e){ return null; }
}
...
function renderCategoria(){
  content.innerHTML=`<div class="screen"><div class="fill">
    <div class="progress" style="flex:0 0 auto"><i style="width:50%"></i></div>
    <div class="eyebrow" style="margin:0 4px 8px;flex:0 0 auto"><b>Agregar caso</b> <span class="grp">· Categoría</span></div>
    <h2 class="q" style="margin:0 4px 14px;flex:0 0 auto">Selecciona el tipo de caso</h2>
